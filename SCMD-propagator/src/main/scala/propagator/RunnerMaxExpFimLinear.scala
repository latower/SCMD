...
object RunnerMaxExpFimLinear extends App with CPModel {
  ...
  parser.parse(args, Config()) match {
    case Some(config) =>
      System.err.println("Start MaxExpFimLinear on " + config.bddFile.getAbsolutePath + " and "
      ...
      val wbddC = new WbddConstraintLinear(bdd, X, minExp)
    ...
    new scopt.OptionParser[Config]("MaxExpFim") {
      ...
      opt[String]("branching") optional () valueName ("<strategy>") action { (x, c) =>
        ...
        if (List("top-zero", "top-one",
          ...
          "item-support") contains x) success else failure("unknown <strategy>")
      } text ("variable/value selection strategy [top-zero, top-one, derivative-zero, derivative-one, bottom-zero, bottom-one], default: top-zero")
